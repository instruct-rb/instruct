module Instruct::Model
  # Handles executing middleware chain.
  class MiddlewareChain
    # @param middlewares [Array<#call(req, next:), MiddlewareChain>] An array of middleware objects. This can be a mix of classes, instances, procs, or other middleware chains.
    ...
    # @api private
    # Don't use. This is internally used to call the next middleware in the chain.
    def call(req)
      ...
      if @stack_ptr >= @middlewares.size
        raise RuntimeError, "Middleware chain exhausted, last object should not have called next"
      end
      ...
      middleware.is_a?(Instruct::Model::MiddlewareChain) ? middleware.execute(req) : middleware.call(req, next: self)
    end
