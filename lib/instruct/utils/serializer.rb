...
module Instruct::Serializer
    ...
    def self.dump(transcript_or_completion)
      YAML.dump(transcript_or_completion)
    end

    def self.load(transcript_or_completion_bytes, permitted_classes: nil)
      permitted_classes ||= []
    ...
    def self.instruct_classes
      # find all clasess in the Instruct module
      Instruct.constants.map { |const| Instruct.const_get(const) }.select { |const| const.is_a?(Class) }
    end
    PERMITTED_CLASSES = instruct_classes
    ...
    def self.transform_ast(node, permitted_classes)
